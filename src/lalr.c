    ...
    do {
        ...
        for (i = 0; i < nprods; i++) {
            ...
            for (s = gram[i]->body + 2; (g = *s++) != 0; ) {
                if (isterm(g)) {
                    if (!tbit(first[h - NB], g)) changed = YES;
                    sbit(first[h - NB], g);
                    break;
                }
                changed |= orbits(first[h - NB], first[g - NB]);
                if (!nullable[g - NB]) break;
            }
...
void comp_empty(){
    ...
    do {
        ...
        for (i = 0; i < nprods; i++) {
            ...
            if ((rhs == 0 || gsym[rhs]->assoc & F_EMPTY) && !(gsym[lhs]->assoc & F_EMPTY)) {
    ...
    if (debug) {
        ...
        for (i = 0; i < nnonts; i++) {
            if (gsym[NB+i]->assoc & F_EMPTY) fprintf(vfp, " %s", gsym[NB+i]->name);
        }
        fprintf(vfp, "\n");
    }
}
...
bool sameset(LR1 *p, LR1 *t){
    while (t != NULL) {
        if (p == NULL || p->item != t->item) return NO;
        p = p->next;
        t = t->next;
    }
...
LR1 *make_state(LR1 *items){
    ...
    for (p = items; p != NULL; p = p->next) {
        g = *p->item;
        if (!isterm(g)) tail = find_empty(tail, g);
    }
...
int cmp_LR1(LR1 *x, LR1 *y){
    int w;
    if ((w = (int)x->item[-1] - (int)y->item[-1]) != 0) return (w);
    ...
    for (p = states; p != NULL; p = p->next) {
        ...
        for (tp = tmplist; tp != NULL; ) {
            ...
            for (lp = states_thru[g]; lp != NULL; lp = lp->next) {
                if (sameset(((State *)lp->elem)->items, sublist)) break;
            }
            if (lp != NULL) {
                free_items(sublist);
                q = lp->elem;
            ...
            if (ni >= height(nextst)) die("Too many actions in one state");
            nextst[ni++] = q;
        }
...
void print_state(int st){
    ...
    for (s = statev[st]->shifts, r = statev[st]->reduce; ; ) {
        if ((*s) != NULL && (*s)->thru < r->sym) {
            ...
            if (isreduceonly(*s)) fprintf(vfp, " and reduce (%d)", (*s)->reduce[0].num);
            fprintf(vfp, "\n");
            s++;
        } else {
...
void fill_reduce(){
    ...
    for (p = states; p != NULL; p = p->next) {
            ...
            for (m = 0; (t = p->shifts[m]) != NULL; m++) {
                ...
                if (tbit(alook, e)) {
                    ...
                    if (rel == NON_ASSOC) {
                        cbit(alook, e);
                        p->shifts[m] = RUBOUT;
                        if (nr >= height(tmpr)) die("Too many actions in one state");
                        tmpr[nr].sym = e;
                        tmpr[nr++].num = -1;
#if 0
...
#endif
                    if (pnum < tmpr[j].num) tmpr[j].num = pnum;
        ...
        *r = NULL;
        ...
        for (j = 0; j < nr; j++) {
            if (tmpr[j].num >= 0) visited[tmpr[j].num] = 1;
        }
        /* Register tmpr */
...
global void comp_lalr(){
    ...
    if (nsrerr != expected_srconf || nrrerr != 0) {
        ...
        if (nrrerr != 0) fprintf(stderr, "%s %d reduce/reduce", conj, nrrerr);

...
Gsym intern_gsym(char *s, bool term){
    ...
    if (term || p->name[0] == '\'') {
        if (nterms >= MAXTERM) die("Too many terminals");
        gsym[nterms] = p;
        p->code = nterms++;
        p->assoc = A_UNDEF;
        p->prec = 0;
        ...
        if (nnonts >= MAXNONT) die("Too many nonterminals");
        gsym[NB + nnonts] = p;
        p->code = NB + nnonts++;
        p->assoc = A_UNDEF;
        p->prec = 0;
...
char *copyact(Gsym *g, int n, int delm, char *attrname[]){
    ...
    while ((tok = raw_gettoken()) != delm || ct > 0) {
        ...
        semval:
            if (type == NULL) type = gsym[g[v]]->type;
...
global void do_grammar(){
    ...
    while (t != MARK && t != EOF) {
        ...
        gbuf[i++] = 0;
        if (!action) {
            if (i > 2 && gsym[gbuf[0]]->type != NULL && gsym[gbuf[0]]->type != gsym[gbuf[1]]->type) {

...
LANGMAP *lang_byname(char *lang){
    ...
    for (p = langmap; p->id >= 0; p++) {
        if (strcmp(p->name, lang) == 0) return p;
    }
    return NULL;
}
...
LANGMAP *lang_byyaccext(char *yext){
    struct langmap *p;
    for (p = langmap; p->id >= 0; p++) {
        if (strcmp(p->yext, yext) == 0) return p;
    }
    return NULL;
}
...
bool metamatch(char *text, char *keyword){
    return *text == metachar && strncmp(text + 1, keyword, strlen(keyword)) == 0;
}
...
void gen_tokenval(){
    ...
    for (; fgets(line, sizeof(line), pfp); plineno++) {
        ...
        if (n >= height(mac)) proto_error("$tokenval: macro too large", NULL);
...
void gen_reduce(){
    ...
    for (; fgets(line, sizeof(line), pfp); plineno++) {
        ...
        if (n >= height(mac)) proto_error("$reduce: macro too large", NULL);
        mac[n] = talloc(strlen(line) + 1);
        strcpy(mac[n], line);
        n++;
    }
...
void parser_generate(){
    ...
    for (; fgets(line, sizeof(line), pfp); plineno++) {
        ...
        if (skipmode) {
            if (metamatch(skipsp(line), "endif")) skipmode = NO;
            continue;
        }

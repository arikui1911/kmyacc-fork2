...
static void print_table(){
    ...
    for (j = 0; j < nterms; j++) {
        for (i = 0; i < nnonleafstates; i++) {
            if (!isvacant(term_action[i][j])) break;
        }
        if (i < nnonleafstates) {
...
static void extract_common(){
    ...
    for (;;) {
        ...
        for (j = 0; j < maxaux->preimage->len; j++) {
            int cl = maxaux->preimage->classes[j];
            if (class2nd[cl] < 0) class2nd[cl] = maxaux->index;
        }
...
static void pack_table(short **transit, int nrows, int ncols, bool dontcare,
    ...
    for (ii = 0; ii < nrows; ii++) {
        ...
        for (j = 0; j < poolsize; j++) {
            ...
            if (!dontcare) {
                ...
                for (h = 0; h < ii; h++)
                    if (base[trow[h].index] == base[i]) goto next;
            }

            for (k = trow[ii].mini; k < trow[ii].maxi; k++) {
                if (!isvacant(transit[i][k])) {
                    if (jj >= poolsize) die("can't happen");
                    if (check[jj] >= 0 && !(dontcare && actpool[jj] == transit[i][k])) goto next;
                }
                jj++;
            }
            break;
        ...
        if (jj >= actpoolmax) actpoolmax = jj;
    ok:
        ;
    }
    ...
    for (i = 0; i < nrows; i++) {
        for (j = 0; j < ncols; j++) {
            if (dontcare && isvacant(transit[i][j])) continue;
            if ((unsigned)(base[i] + j) < actpoolmax && check[base[i] + j] == (checkrow ? i : j)) {
...
static void authodox_table(){
    ...
    for (j = 0; j < nnonts; j++) {
        ...
        for (i = 0; i < nnonleafstates; i++) {
            ...
            if (nonterm_goto[i][j] == default_goto[j]) nonterm_transposed[j][i] = VACANT;
        }
    }
...
static int cmp_preimage(const Preimage *x, const Preimage *y){
    ...
    for (i = 0; i < x->len; i++) {
        if (x->classes[i] != y->classes[i]) return x->classes[i] - y->classes[i];
    }
    return 0;
}
...
static void comp_preimages(){
    ...
    for (i = 0; i < nclasses; i++) {
        for (j = 0; j < nterms; j++) {
            int s = class_action[i][j];
            if (s > 0) primv[s]->classes[primv[s]->len++] = i;
        }
    }
